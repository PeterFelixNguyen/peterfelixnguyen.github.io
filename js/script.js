...
$(document).ready(function() {
  /* Initialize the LazyLoader (Thank you, Mika Tuupola for creating this) */
    ...
    /* On some browsers like Chrome (87.0.4280.88 Official Build 64-Bit), the browser will attempt to
       ...
       The code below disables this auto-scrolling if it is a feature enabled in the browser. */
  ...
  /* Remove the duplicate id (temporary anchor) 
     to make the first tab "clickable"
     
     The delay is necessary to ensure the anchor is deleted
     only AFTER the history.pushState() 
     
     But this delay causes problems when you click the first tab before 
     the temporary anchor is finished being deleted, resulting in 
     the first tab pane failing to show when you click its tab */
  setTimeout(function() {
    $("span.delete-on-load").remove();
  }, 300);

  /* Handle the click event on the vertical tabs */
  $(document.body).on("click", "a[data-toggle]", function(event) {
    /* Delete the temporary anchor with the duplicate id again, just in
       case the user clicks on the first tab before the duplicate id is 
       finished being deleted.
       
       This is essential because the duplicate id will interfere with tab functionality 
       (i.e. the first tab pane will not show because it thinks the temporary anchor is the tab pane).

       One would expect this trick to fail because Bootstrap's tab showing function 
       might occur BEFORE this click function. However, this trick still works
       because I suspect that the tab showing function actually occurs AFTER this 
       click function. However, I am not sure if this is true and if this solution 
       will always work. This is why I also delete the duplicate id on page load. */
    $("span.delete-on-load").remove();
...
/*
   ...
   Consider migrating the JQuery hover function to a CSS only method with media queries (test before release)
*/
...
/*
   Function to play GIF when mouse hovers on a screenshot
   This version DOES NOT check if an image exists before attempting to animate
*/
$("img.play-gif").hover(function() {
  /*
     Hover enter 
  */
  
  /* Get the original image src (.png) */
  var oldSrc = $(this).data('src');
  /* Get the associated animated image src (.gif) */
  var newSrc = oldSrc.replace('.png', '.gif');
  /* Change the original image to the animated image src */
  ...
  */
  ...
  /* Get the hover-state image src (.gif) */
  var srcName = $(this).data('src');
  /* Change the image to the static image src */
  $(this).attr("src", srcName.replace('.gif', '.png'));
});


/*
   Function to play GIF when mouse hovers on a screenshot
   This version checks if an image exists before attempting to animate
*/
$("img.play-gif-nullsafe").hover(function() {
  /* 
     Hover enter
  */
  ...
  /* Assign the image src to the Image instance */
  imageCheck.src = newSrc;
  /* Assign img to a variable */
  var imgElement = $(this);
  /* Set a short delay to ensure Image instantiates before performing psuedo null check */
  setTimeout(function() {
    /* Check if the image exists by checking width */
    if (imageCheck.width != 0) {
      /* If animated image exists, change the original image to the animated image src */
  ...
  /* 
     Hover leave
     Note: there is no need to nullcheck here, no a need to setTimeout
  */
  
  /* Get the hover-state image src (.gif) */
  var oldSrc = $(this).data('src');
  /* Get the associated static image src (.png) */
  var newSrc = oldSrc.replace('.gif', '.png');
  /* Assign img to a variable */
  var imgElement = $(this);
  /* Set a delay longer than onHover delay to ensure image resets are NEVER overridden by the onHover timeout */
  setTimeout(function() {
    /* Change the image to the static image src */
    imgElement.attr("src", newSrc);
  }, 100);
...
$("h2.collapse-toggle").on('click', function(){
  ...
  var iSelector = $(this).find("span.glyphicon");
